...
\[ \forall x,y \in X, \forall t \in [0,1]: \quad tx + (1-t)y \in X \]
\end{definicion}
...
\textbf{Ejercicio:} Reproduce las pruebas del teorema de Poincaré para arcos cualquiera.

Recordamos que vimos:
\begin{align*}
    [\alpha]([\beta][\gamma]) &= ([\alpha][\beta])[\gamma] \quad \forall \alpha, \beta, \gamma \in C_{x_0}(X) \\
    [c_{x_0}][\alpha] &= [\alpha] \quad \forall \alpha \in C_{x_0}(X) \\
    [\alpha^{-1}][\alpha] &= [c_{x_0}] \quad \forall \alpha \in C_{x_0}(X)
\end{align*}
Además, si $[\alpha] = [\alpha']$ y $[\beta] = [\beta'] \implies [\alpha * \beta] = [\alpha' * \beta']$ para todo $\alpha, \beta \in C_{x_0}(X)$.

Probar que esta propiedad se cumple de igual manera para arcos, además que para el elemento neutro:
\begin{enumerate}
    ...
    \item[d)] $\alpha^{-1} * \alpha \simeq_{\{0,1\}} c_{x_0}$ y $\alpha * \alpha^{-1} \simeq_{\{0,1\}} c_{x_1}$.
\end{enumerate}
puto 
\begin{proposicion}{Inverso homotópico de caminos}{inverso_homotopico}
Sea $X$ espacio topológico, $x_0, x_1 \in X$. Sean $\alpha, \beta \in C_{x_0, x_1}(X)$ que verifican que $\alpha \simeq_{\{0,1\}} \beta$, entonces $\alpha^{-1} \simeq_{\{0,1\}} \beta^{-1}$. (Ejercicio para casa).
\end{proposicion}

 david
\textbf{Ejercicio:} Sean $\alpha, \beta \in C_{x_0, x_1}(X)$, prueba que $(\alpha * \beta)^{-1} = \beta^{-1} * \alpha^{-1}$.
\begin{proof}
...
\begin{observacion}{}{obs_inversos}
\begin{enumerate}
    \item Volviendo a $\pi_1(X, x_0)$, vimos que el inverso de un elemento $[\alpha] \in \pi_1(X, x_0)$ es $[\alpha^{-1}]$. Luego $[\alpha]^{-1} = [\alpha^{-1}]$.
    \item Si $\alpha \in C_{x_0}(X)$, entonces $[\alpha^{-1}][\alpha] = [\alpha][\alpha^{-1}] = [c_{x_0}]$, y $[c_{x_0}][\alpha] = [\alpha][c_{x_0}] = [\alpha]$. Además, $([\alpha]^{-1})^{-1} = [\alpha]$.
...
\begin{definicion}{Concatenación conjunta}{concat_conjunta}
...
\end{cases}
\]
Esta aplicación es continua. Como ejercicio, probar que $[\alpha * \beta * \gamma] = [(\alpha * \beta) * \gamma] = [\alpha * (\beta * \gamma)]$.
\end{definicion}
...
\begin{definicion}{Isomorfismo de cambio de base}{def_cambio_base}
...
\begin{teorema}{Isomorfismo de cambio de base}{teo_cambio_base}
$P_\omega$ es un isomorfismo de grupos.
\end{teorema}

\begin{proof}
1) $P_\omega$ está bien definido. Necesitamos probar que si $[\alpha]=[\alpha']$, entonces $[\omega^{-1} * \alpha * \omega] = [\omega^{-1} * \alpha' * \omega]$.
Si $\alpha \simeq_{\{0,1\}} \alpha'$, entonces $\alpha * \omega \simeq_{\{0,1\}} \alpha' * \omega$. Aplicando lo anterior se tiene:
\[
\omega^{-1} * (\alpha * \omega) \simeq_{\{0,1\}} \omega^{-1} * (\alpha' * \omega) \implies \omega^{-1} * \alpha * \omega \simeq_{\{0,1\}} \omega^{-1} * \alpha' * \omega
\]
...
&= [(\omega^{-1} * \alpha) * (\omega * \omega^{-1}) * (\beta * \omega)] \quad (\text{pues } \omega * \omega^{-1} \simeq_{\{0,1\}} c_{x_0}) \\
&= [(\omega^{-1} * \alpha * \omega) * (\omega^{-1} * \beta * \omega)] \\
&= [\omega^{-1} * \alpha * \omega] \cdot [\omega^{-1} * \beta * \omega] = P_\omega([\alpha]) \cdot P_\omega([\beta])
\end{align*}

3) $P_\omega$ es biyectiva. Comprobaremos que $P_\omega^{-1} = P_{\omega^{-1}}$.
Para todo $[\gamma] \in C_{x_1}(X)$:
\[
P_\omega(P_{\omega^{-1}}([\gamma])) = P_\omega([(\omega^{-1})^{-1} * \gamma * \omega^{-1}]) = [\omega^{-1} * (\omega * \gamma * \omega^{-1}) * \omega] = [c_{x_1} * \gamma * c_{x_1}] = [\gamma]
\]
Análogamente, para todo $[\alpha] \in C_{x_0}(X)$:
\[
P_{\omega^{-1}}(P_\omega([\alpha])) = P_{\omega^{-1}}([\omega^{-1} * \alpha * \omega]) = [\omega * (\omega^{-1} * \alpha * \omega) * \omega^{-1}] = [c_{x_0} * \alpha * c_{x_0}] = [\alpha]
\]
\end{proof}

\begin{proposicion}{}{cor_arco}
Si $X$ es arcoconexo y $x_0, x_1 \in X$, entonces $\pi_1(X, x_0) \cong \pi_1(X, x_1)$.
\end{proposicion}

\begin{nota}{Grupo fundamental}{nota_grupo_fund}
...
Consideremos ahora una aplicación continua. Ejercicio: Sea $X$ e.t., $x_0, x_1 \in X$ y sean $\omega_0, \omega_1 \in C_{x_0, x_1}(X)$. Prueba que si $\omega_0 \simeq_{\{0,1\}} \omega_1$ entonces $P_{\omega_0} = P_{\omega_1}$.

\begin{definicion}{Homomorfismo inducido}{def_hom_ind}
Sea $f: X \to Y$ continua, sea $x_0 \in X$ y sea $y_0 = f(x_0)$. Definimos:
\begin{align*}
...
\begin{teorema}{Propiedades del homomorfismo inducido}{teo_hom_ind}
$f_*$ es un homomorfismo de grupos.
\end{teorema}

\begin{proof}
1) Bien definido. ¿$f \circ \alpha$ es un lazo?
Si $[\alpha] = [\alpha'] \implies \alpha \simeq_{\{0,1\}} \alpha' \implies f \circ \alpha \simeq_{\{0,1\}} f \circ \alpha'$, lo que significa que $[f \circ \alpha] = [f \circ \alpha']$.
...
\textbf{Ejercicio:} Sea $x_0 \in X$, probar que $(g \circ f)_* = g_* \circ f_*$.
